...
class DWA:
    ...
    def window(self, v, w):
        ...
        # 产生速度空间窗口
        # 最大速度，最大角速度限制
        vwlimit = [self.config.min_speed, self.config.max_speed, -self.config.max_yawrate, self.config.max_yawrate]
        # 基于当前速度、最大加速度、最大角加速度的速度限制
        vwmove = [v - self.config.max_accel * self.config.dt, v + self.config.max_accel * self.config.dt,
                  w - self.config.max_dyawrate * self.config.dt, w + self.config.max_dyawrate * self.config.dt]
        # 取交集
    ...
    def vevaluate(self, v):
        ...
        """
        # 归一化速度损失
        evaluation = (self.config.max_speed - v) / self.config.max_speed
    ...
    def oevaluate(self, prex, prey, obs):
        ...
        evaluation = self.config.robot_size / (
                    distance - self.config.robot_size) if distance > self.config.robot_size else np.inf
        return evaluation
    ...
    def plan(self, x, goal, ob):
        ...
        for v in np.append(np.arange(vw[0], vw[1], (vw[1] - vw[0]) / self.config.vstep), vw[1]):
            for w in np.append(np.arange(vw[2], vw[3], (vw[3] - vw[2]) / self.config.wstep), [0, vw[3]]):
                prex, prey, prealpha = self.prediction(x[0], x[1], v, w, x[2])
                gevaluation = self.config.to_goal_cost_gain * self.gevaluate(prex, prey, prealpha, goal[0], goal[1])
                vevaluation = self.config.speed_cost_gain * self.vevaluate(v)
                oevaluation = self.config.obstacle_cost_gain * self.oevaluate(prex, prey, ob)
        ...
        if goal[2] == 0.001 and math.hypot(goal[0] - x[0], goal[1] - x[1]) < self.config.threshold * 5 and now_v > self.config.max_speed / 3:
                bestv = min(now_v - self.config.max_accel / 5 * self.config.dt, bestv)
                print("\n\n\n\n\n\n\n\n\n")
                print('I am going to turn!!!!!')
                print("\n\n\n\n\n\n")

        if goal[2] == 0.001 and math.hypot(goal[0] - x[0], goal[1] - x[1]) < self.config.threshold * 2 and now_v > self.config.max_speed / 5:
                bestv = min(now_v - self.config.max_accel / 7 * self.config.dt, bestv)
                print('almost achieve turning')
                
        if goal[2] == 0.002 and math.hypot(goal[0] - x[0], goal[1] - x[1]) < self.config.threshold and now_v > self.config.max_speed / 5:
                bestv = min(now_v - self.config.max_accel / 7 * self.config.dt, bestv)
                bestw = 0
                print('almost achieve turning')
